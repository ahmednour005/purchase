...
<head>
  ...
  <title>EECGroup |  @lang('site.services')</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row">
                    ...
                    <form action="{{route('service.update',$data->id )}}" method="Post">
                        ...
                        <div class="form-group">
                            <label for="Add Service ">@lang('site.edit_service') </label>
                            <input type="text" name="service_name" value="{{$data->service_name}}" class="form-control" required="" oninvalid="this.setCustomValidity('@lang('site.check_service')')"  oninput="setCustomValidity('')" >
                        </div>
                        <div class="form-group">
                            {{ method_field('PUT') }}
            ...
            @endif
                    ...
                    <div class="col-md-4">
                        ...
                        <form action="{{route('service.store')}}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="service_name" value="" class="form-control" required="" oninvalid="this.setCustomValidity('@lang('site.check_service')')"  oninput="setCustomValidity('')" >
                            </div>
                            <div class="form-group">
                    ...
                    @endif
                    @if(auth()->user()->hasPermission("service_create") ||
                     auth()->user()->hasPermission("service_update") )
                     @if($data)
                        <div class="col-md-8">
                        @else
                        @if(auth()->user()->hasPermission("service_create"))
              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr style="text-align:center;">
                    ...
                    @if(auth()->user()->hasPermission("service_update") ||
                    auth()->user()->hasPermission("service_delete"))
                  ...
                  <tbody>
                    @foreach($services as $service)
                        ...
                        <tr>
                            <td>{{$service->id}}</td>
                           <td>{{ $service->mainGroup->group_name}}</td>
                            <td>{{$service->service_name}}</td>
                            <td>
                                @foreach($sup_service as $s_ser)
                                    @if($s_ser->service_id == $service->id)
                            ...
                            <td>
                                <div class="service-option">
                                    @if(auth()->user()->hasPermission("service_update"))
                                       <a href="{{route('service.edit',$service->id)}}" class=" btn btn-warning"><i class="fa fa-edit"></i> @lang('site.edit') </a>
                                    @endif
                                    @if(auth()->user()->hasPermission("service_delete"))
                                       <a  class=" btn btn-danger"  data-service_id="{{$service->id}}" data-toggle="modal" data-target="#service_delete"><i class="fa fa-trash-alt"></i>  @lang('site.delete')  </a>
                                    @endif
                                </div>
                            </td>
                            @endif
    ...
    <div class="modal fade text-center" id="service_delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" dir="rtl" >
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle"> @lang('site.delete_service') </h5>
...
<script>
    ...
    $('#service_delete').on('show.bs.modal',function(event){
        ...
        $('.modal #service_id').val(serviceid);
    })
</script>
